...
struct llm_graph_context {
    ...
    // folded weights that share an activation also share its transform; ggml does
    // no common-subexpression elimination, so build it once per (input, rotation)
    ...
    // rotate a folded weight's activation into the basis its weights were stored in
    ggml_tensor * hadamard_transform(
              ggml_tensor * w,
