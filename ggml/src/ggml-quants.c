...
// PTQ1_0 trit staging: 5 trits per byte over the qs area, 4 per byte over qh.
// The 24-byte qs covers 120 values in stages of 32/16/8 bytes; what staging
// cannot cover falls through to qh, so the stages are generalised from TQ1_0's
// 48-byte qs (where they reduce to exactly its original 32-then-16 behaviour).
