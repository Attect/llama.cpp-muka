...
// PQ2_0: the group-128 ternary packing above, republished by PrismML under its
// own type id for Bonsai 2. Layout is unchanged, so the block is aliased.
#define QK_PQ2_0 QK2_0
typedef block_q2_0 block_pq2_0;
static_assert(sizeof(block_pq2_0) == sizeof(ggml_half) + QK_PQ2_0 / 4, "wrong pq2_0 block size/padding");

// PTQ1_0: PrismML ternary at group size 128. Same base-3 trit codec as TQ1_0, but
// one fp16 scale per 128 weights instead of per 256 -> 1.75 bpw, and lossless for
// checkpoints that are already ternary at group 128 (a 256-wide scale would have to
